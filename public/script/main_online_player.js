// script/main_online_player.js (Final Complete Version)
...
// ★時間管理用
let remainingTime = { black: 1200, white: 1200 }; 
...
window.addEventListener("load", () => {
    ...
    // ★ここでの render はデータがないのでスキップされるが安全
    render();
...
function setupSocketListeners(myUserId) {
    ...
    socket.on('restore game', (savedState) => {
        ...
        if (savedState.isCaptureRestricted !== undefined) {
            window.isCaptureRestricted = savedState.isCaptureRestricted;
        }
    ...
    socket.on('skill activate', (data) => {
        ...
        playSkillEffect(data.turn); // 演出再生
...
function executeMove(sel, x, y, doPromote, fromNetwork = false) {
    ...
    if (!fromNetwork) {
        ...
        socket.emit('shogi move', { sel: sel, x: x, y: y, promote: doPromote, gameState: sendState });
    }
...
// ★必殺技カットイン演出（画像パス修正版）★
function playSkillEffect(playerColor) {
...
function undoMove() {
    ...
    else window.isCaptureRestricted = false;
}
